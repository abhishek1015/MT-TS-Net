...
output_map_file = data_basedir +  project_str + '/ensembl_gene_id_to_gene_mapping_pwonly.json'
...
pw_file = data_basedir + project_str + '/pathway_gene_list.csv'
symbol_dict={}
row_dict={}
genes=[]
expressions=[] 

pw = pd.read_csv(pw_file, delimiter=',')
pw_list = [x.strip() for x in pw.columns.tolist()] 
...
for g in range(len(symbols)):
    if 'symbol' in symbols[g].keys():
        if symbols[g]['symbol'] in gene_above_threshold[gene_above_threshold==1].keys():
            if symbols[g]['symbol'] in pw_list:
