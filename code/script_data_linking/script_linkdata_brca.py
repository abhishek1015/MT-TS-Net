...
casewise = int(input('press 1 for casewise: '))
...
info_columns = ['filename', 'duration', 'event', 'ajcc_pathologic_stage', 'ajcc_pathologic_n', 'ajcc_pathologic_m', 'ajcc_pathologic_t', 'morphology', 'primary_diagnosis', 'age_at_index', 'tcga_subtype', 'immune_subtype', 'MDSC', 'CAF', 'M2', 'Exclusion', 'Dysfunction', 'ge_file']
...
with open(base_data_dir + project_id[project] + csv_filename, mode='w') as csv_file:
    ...
    for case in range(len(svsfile)):
        ...
        if casewise==1:
            rowid = case_submitter_id
            if rowid in seencase.keys():
                continue
            else:
                seencase[rowid]=1
        else:
