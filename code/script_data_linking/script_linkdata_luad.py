...
project_id ={'1': 'tcga-brca','2': 'tcga-luad', '3': 'tcga-gbm'}
base_data_dir = '/data/dubeyak/Cancer-research/code/data/'
seg_dir = '/data/Jiang_Lab/Data/'
project = input('enter project id: press 1 for tcga-brca and press 2 for tcga-luad and press 3 for tcga-gbm: ')
...
tidefile = pd.read_csv(base_data_dir + 'tide-results/Tumor_Dysf_Excl_scores/TCGA.LUAD.RNASeq.norm_subtract.OS_base', delimiter='\t')
...
with open(base_data_dir + project_id[project] + '/linked_data_aug.csv', mode='w') as csv_file:
    ...
    for case in range(len(svsfile)):
        ...
        if len(vital_status)>0:
            ...
            if (duration == "\'--" or age_at_index.values[0] == "\'--"):
                continue
            ...
            if _patch_count.values[0] < patch_count_threshold:
                  continue
